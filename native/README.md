...
recognition state and `openflow-llm-worker` owns language-model scoring and conservative
cleanup. Each is a persistent subprocess, so a model is loaded once and reused across
...
Both adapters may be enabled in one build. OpenFlow does not fetch either
...
- LLM cleanup treats model scores as evidence, not authority. Formatting and exact
  adjacent duplicates are accepted; protected text and high-confidence lexical content
  cannot be rewritten. Medium-confidence lexical edits need a 0.5 nat-per-source-token
  advantage, and low-confidence edits need a non-negative advantage. llama.cpp lexical
  generation is deterministic, grammar-constrained, bounded, and source-range checked
  before the same independent gate runs.
